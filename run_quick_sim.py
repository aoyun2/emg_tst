...
_ap.add_argument("--seconds", type=float, default=5.0,
                 help="Seconds of data to aggregate for motion matching and simulation (default: 5)")
...
if args.data:
    # Pre-windowed samples_dataset.npy with disjoint windows (stride = seq_len).
    # Take only enough windows to cover --seconds, then concatenate.
    ...
    n_wins_needed = max(1, int(np.ceil(max_frames / x_wins.shape[1])))
    x_wins  = x_wins [:n_wins_needed]
    y_seqs  = y_seqs [:n_wins_needed]
    x_windows_for_inference = x_wins   # (n_wins_needed, seq_len, n_vars)
    ...
    source_label = f"samples_dataset ({len(x_wins)} windows × {x_wins.shape[1]} frames, {args.data})"
...
if args.checkpoint and x_windows_for_inference is not None:
    ...
    # Windows are disjoint (stride = seq_len), so concatenate full predictions.
    _n_wins = len(x_windows_for_inference)
    pred_chunks = [
        _model_predict(_model, _scaler, x_windows_for_inference[i], device)
        for i in range(_n_wins)
    ]
    knee_model_pred = np.concatenate(pred_chunks).astype(np.float32)[:len(knee_query)]
    knee_good_pred = np.clip(knee_model_pred, 0.0, 180.0)
    good_rmse = float(np.sqrt(np.mean((knee_good_pred - knee_query) ** 2)))
    print(f"Model RMSE vs GT : {good_rmse:.2f} °  ({_n_wins} windows × {x_windows_for_inference.shape[1]} frames)")
