...
# Where the runs and the built artifacts live varies by machine, so it is
# configuration and is read from the environment rather than written here.
RUNS = pathlib.Path(os.environ.get("EMG_TST_RUNS", "runs")).expanduser()
ARTIFACTS = pathlib.Path(os.environ.get("EMG_TST_ARTIFACTS", "artifacts")).expanduser()
ZIP = ARTIFACTS / ("Toward_the_use_of_simulated_environments_to_evaluate_"
                   "sEMG_informed_knee_angle_prediction.zip")
