...
($\rho=+0.327$,
$p=0.253$). A two-segment fit located a turn at
12.97$^{\circ}$ (95\% CI
10.38 to 16.22$^{\circ}$). Above it, worse prediction
gave more instability (slope
+0.01304 score-seconds per
degree, 95\% CI 0.00694 to 0.01949). Below it
...
above roughly 13$^{\circ}$. Below that the
...
($n=90$). Trials 1--3 supplied participant-specific
...
convergence. The reported path ran 400000 steps and
...
changed participant RMSE by -0.085$^{\circ}$
relative to the aligned model (95\% CI
-0.202 to 0.030$^{\circ}$;
$t(65)=-1.42$, $p=0.160$). The surrogate controls
(Supplementary Table~\ref{tab:controls}) agreed. The recorded signal outperformed all 3 surrogates in paired within-participant comparisons (smallest margin 0.288$^{\circ}$, all $p\leq1.68\times10^{-7}$).
...
the residual-fusion and no-sEMG conditions was +0.0084
score-seconds (95\% CI -0.0032 to 0.0199;
...
+0.01304 score-seconds per
degree, 95\% CI 0.00694 to 0.01949).
...
95\% CI -0.01091 to -0.00341). Both intervals exclude
...
+0.042 on average. Excess instability therefore approaches that
...
approximately 12.97$^{\circ}$ of prediction
...
12.97$^{\circ}$ (95\% CI
10.38 to 16.22$^{\circ}$) a less accurate model
...
5.09$^{\circ}$, sits below that band.
...
periodic-signal surrogate cannot be expected to do. The recorded signal outperformed all 3 surrogates in paired within-participant comparisons (smallest margin 0.288$^{\circ}$, all $p\leq1.68\times10^{-7}$).
...
Circular shift & +0.058 & 0.005 to 0.113 & $0.041$ & 0.17 & +0.288 ($p=1.68\times10^{-7}$) \\
Participant swap & -0.011 & -0.048 to 0.026 & $0.556$ & -0.03 & +0.357 ($p=3.00\times10^{-10}$) \\
Phase randomized & -0.022 & -0.055 to 0.008 & $0.167$ & -0.07 & +0.368 ($p=9.26\times10^{-12}$) \\
\bottomrule
\end{tabular}
\end{table}
